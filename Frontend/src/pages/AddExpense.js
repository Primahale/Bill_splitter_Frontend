...
const AddExpense = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://bill-splitter-app-r1fi.onrender.com/api/events', eventData);
      alert('Event created successfully!');
      navigate('/');
    } catch (error) {
      console.error('Error creating event:', error.response ? error.response.data : error.message);
      alert(`Error: ${error.response?.data?.message || 'An error occurred while creating the event.'}`);
    }
  };
